...
from ..core.exceptions import FileWriteError, GSettingsError, ThemeApplicationError
...
class GTKHandler(BaseHandler):
    ...
    def _apply_gtk3_theme(self, theme_name: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                "gsettings command not found, skipping GTK theme application"
            )
            return False
        except Exception as e:
    ...
    def _generate_libadwaita_css(self, theme_data: ThemeData) -> str:
        ...
        if theme_data.css_content:
            css_parts.extend(["", "/* Additional CSS */", theme_data.css_content])
    ...
    def is_available(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["gsettings", "--version"], capture_output=True, check=False
            )
