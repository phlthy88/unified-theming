...
def validate_css_syntax(css_content: str) -> List[str]:
    ...
    try:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if brace_count < 0:
                errors.append(
                    f"Line {line_num}: Extra closing brace without matching opening brace"
                )
            ...
            if re.search(r"\w+\s*:\s*[^;]+{", line):
                errors.append(
                    f"Line {line_num}: Property seems to be missing semicolon"
                )

        if brace_count > 0:
            errors.append(
                f"CSS structure error: {brace_count} unclosed block{'s' if brace_count > 1 else ''}"
            )
...
def validate_theme_structure(theme_path: Path) -> List[str]:
    ...
    if not has_gtk_support:
        errors.append(
            f"Theme has no GTK support directories. Expected one of: {', '.join(gtk_dirs)}"
        )
...
def validate_color_palette(color_palette: ColorPalette) -> List[str]:
    ...
    missing_required = [
        color for color in required_colors if color not in color_palette
    ]
    if missing_required:
