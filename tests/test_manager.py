...
def test_apply_theme_success():
    ...
    with (
        patch.object(manager, "parser", mock_parser),
...
def test_apply_theme_validation_errors():
    ...
    with (
        ...
        assert (
            result.overall_success is True
        )  # app still succeeds despite validation warnings
...
def test_get_current_themes():
    ...
    with (
        patch("unified_theming.core.manager.GTKHandler") as mock_gtk_handler,
