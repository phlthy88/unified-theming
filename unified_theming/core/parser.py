...
from .types import (
    ThemeInfo, ValidationResult, ValidationMessage, ValidationLevel,
    Toolkit, ColorPalette
)
from .exceptions import (
    ThemeDiscoveryError, ThemeNotFoundError, ThemeParseError, 
    InvalidThemeError, CSSValidationError
)
...
class UnifiedThemeParser:
    ...
    def __init__(self, theme_directories: Optional[List[Path]] = None):
        ...
        self._color_regex = re.compile(r"@define-color\s+([\w-]+)\s+([^;]+);", re.IGNORECASE)
    ...
    def _is_valid_theme_directory(self, theme_path: Path) -> bool:
        ...
        possible_directories = [
            "gtk-2.0", "gtk-3.0", "gtk-4.0", "cinnamon", "gnome-shell",
            "metacity-1", "unity", "xfwm4", "openbox-3", "index.theme"
        ]
    ...
    def validate_theme(self, theme_path: Path) -> ValidationResult:
        ...
        for css_file in css_files:
            try:
                ...
                result.add_error(
                    f"Failed to read CSS file {css_file}: {str(e)}",
                    component="parser"
                )
