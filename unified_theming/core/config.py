...
class ConfigManager:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        logger.info(
            f"ConfigManager initialized with backup directory: {self.backup_dir}"
        )
    ...
    def backup_current_state(self) -> str:
        ...
        theme_name = (
            "current"  # We'll determine actual theme name in a real implementation
        )
        ...
        try:
            ...
            raise BackupError(
                f"Failed to create backup: {str(e)}", backup_path=backup_path
            )
    ...
    def restore_backup(self, backup_id: str) -> bool:
        ...
        try:
            ...
            if not metadata_path.exists():
                raise RollbackError(
                    f"Backup metadata not found: {metadata_path}", backup_id=backup_id
                )
            ...
            for toolkit, files in metadata.get("files", {}).items():
                for original_path_str, backup_path_str in files.items():
                    ...
                    if (
                        backup_file_path.suffixes
                        and backup_file_path.suffixes[-1] == ".gz"
                    ):
                ...
                try:
                    ...
                    if metadata_path.exists():
                        ...
                        backup = Backup(
                            ...
                            backup_path=item,
                            toolkits=[
                                Toolkit(tk) for tk in metadata.get("toolkits", [])
                            ],
                    ...
                    logger.warning(
                        f"Failed to load metadata for backup {item.name}: {e}"
                    )
        ...
        if len(backups) > keep_count:
            ...
            for backup in old_backups:
                try:
                    ...
                    logger.warning(
                        f"Failed to remove old backup {backup.backup_id}: {e}"
                    )
    ...
    def get_backup_info(self, backup_id: str) -> Backup:
        ...
        if not metadata_path.exists():
            raise BackupError(
                f"Backup metadata not found: {metadata_path}", backup_path=backup_path
            )
        ...
        try:
            ...
            raise BackupError(
                f"Failed to load backup info for {backup_id}: {str(e)}",
                backup_path=backup_path,
            )
